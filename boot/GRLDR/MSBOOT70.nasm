...
BIOSIZE					equ		4					; Four contiguous sectors of BIO to load (2048 bytes), according to IDOS v7 boot protocol
BIOMEM					equ		0x700				; Start first sector of BIO v7 with 'MZ'
BIOMEM2					equ		0x900				; Start second sector of BIO v7 with 'BJ'
;
...
;
LoadAddressOffset		equ LoadOff - $$    		; LoaddOff - 0x7C00 => now at 0x3f
...
; NEW code for MSDOS70
	push 	eax									  	; Reserve 4 bytes on Stack, to later load DataStart
...
; NEW code for MSDOS70
	mov		byte [bp+2], 0xe						; Set FAT12/16 LBA type instead of CHS if EBIOS is present. CHS is '90'
; END OF NEW code for MSDOS70
...
; NEW Code for MSDOS70
	mov dword [bp-4], ebx							; Write Start of DataArea to bp-4 for BIO v7 handshake
; End of NEW Code for MSDOS70
; --- First, read the whole root directory into the temporary buffer ---
	mov		eax, dword [bp+RootStartOffset]			; VAR2 = RootStart = [bp + 0x75]
	les		bx, [bp+LoadAddressOffset]				; Target in memory: Values are '00 00 00 20' = [bp + 0x3f] = LoadAddress - ES:BX = 0x2000:0000
; --- cs=ss=ds=0 es=0x2000 ---
	call	DiskRead								; ?? = 0, eax, ES is changed [NOT 0x2000 anymore]
SearchFile:
	les		di, [bp+LoadAddressOffset]				; Target in memory: Values are '00 00 00 20' = [bp + 0x3f] = LoadAddress - ES:BX = 0x2000:0000
...
SearchFileLoop:										; BX=0, CX=0
	...
	je		NoBio									; OR jc 2f, exceeding 64K
...
NotFound:											; 
	...
	jmp 	CommonError								; 
FileFound:
	push	word es:[di+0x1a]						; Save first cluster number of BIO. Offset 1Ah is FAT12/16 cluster number in Directory entry
...
; NEW Code for MSDOS70
	...
	push 	word [0x7A]								; SP = 0x7B..
	push 	word [0x78]								; SP = 0x7B..
	push 	ax										; SP = 0x7B..
	push 	ax										; SP = 0x7B..
	cmp		word [cs:BIOMEM], 0x5a4d				; word must be specified
	jne		NotFound								; 
	cmp		word [cs:BIOMEM2], 0x4a42				; word must be specified
	jne		NotFound								; 
    mov     si, NewLines							; 
    call    Print									; 
	jmp 	BIOSEG:0x0200							; 
