...
;
; FDBOOT.nasm, ported to NASM from FDBOOT.ASM with messages from FDISK5.SKL
; v0.1 (20251230), by deomsh
;
...
;ORG;        mov si, sp       		        ;where this boot record starts - 0:7c00
        push ax
        ...
        mov cx, 100h
;NEW;		rep movsw				; correct according to pi: opcode 'F3 A5'. Repeats while C<>0
;ORG;
        repne movsw     		;relocate to 0:0600 = incorrect/ correct by accident according to pi: opcode 'F2 A5'. Repeats while C<>0 and ZF=0
...
;ORG;        mov bp, si       		        ;save table entry address to pass to partition boot record
next1:
...
;ORG;        mov si, bp       		        ;yes, pass partition table entry address
        db 0eah					; EA = ljump
        dw 7c00h,0				; 00 07 00 00 = 0:0x7c00
;
