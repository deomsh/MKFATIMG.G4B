...
;
; MSBOOT50.nasm = modified MSBOOT40.nasm of original MASM: MSBOOT.ASM converted to NASM
; v0.2 (20251229), by deomsh, with help of Gemini 2.5 PRO, DeepSeek v3 and Copilot (GPT-5)
...
;
;NotToUse?;SEC9_OFFSET		equ START-$$+0	; A temporary storage area for the disk parameter table.
...
;
	...
	SecClus			db	8			;	  DB	  8			; SECTORS PER ALLOCATION UNIT
...
; 	ASSUME	SS:CODE					; NOT used in NASM
	...
	mov 		sp, bp	 			; Sets the stack pointer (SP) to ORIGIN (i.e. 0x7C00). 
;ORG;	mov 		sp, ORIGIN 			; Sets the stack pointer (SP) to ORIGIN (i.e. 0x7C00). 
...
DoLoad:
;===New Code for MS-DOS 5.0: IO.SYS can start at a higer cluster number, but must be still first entry in Root Directory!===
	mov        ax, word [bx + 0x1a]	                ; Read Cluster Number of IO.SYS
...
; The 8-bit registers (AH, AL, BH, BL, CH, CL, DH, DL) are part of the general-purpose registers (AX, BX, CX, DX).
	...
	ret								; Return from subroutine. 
;
SYSMSG  db "Non-System disk or disk error", 0x0d, 0x0a
        db "Replace and press any key when ready...", 0x0d, 0x0a, 0
;%include "boot.cl5" 				; Data area for messages and filenames in file "boot.cl5", shortend in case of MSDOS50
