...
;
; MSBOOT20.nasm <=> MSBOOT30.5.7.nasm with IBM (version2.inc) <=> original MASM: MSBOOT.ASM converted to NASM
; (20251124), by deomsh, with help of Gemini 2.5 PRO, DeepSeek v3 and Copilot (GPT-5)
; v30.5 Compatible with grubutil FAT. Without debug LOADSIZE line
; v30.4 have good code to load actual number of sectors of IO.SYS (max 58)
; v30.3 comes from 30.1 (30.2 was a different approach): THREE Bytes less than with v30.1, without New Code
; OKE, skip 1: mov si, DOS ; DOS follows BIO directly!
;  Still good: lea di, [bx + 0x20] ; after finding IO.SYS = BIO
; NOTEST: => 2: xor ax, ax instead of xor ah. ah
; NEEDED  => 3: cleaning stack Load_Failure before going to CKErr
;
...
IBMLOADSIZE	equ		58		; maximum possible with 0x7C00 - 0x700 - 256 (256 for stack)
; IBMLOADSIZE	equ		47		; Size of IBMLOAD module of MS-DOS/COMPAQ 3.31 in sectors
; IBMLOADSIZE	equ		3		; IBMLOADSIZE equ 3			;J.K. Size of IBMLOAD module in sectors
; IBMLOADSIZE	equ		4		; No difference for 3.31 - Floppie still oke
ROM_DISKRD	equ		2		; ROM_DISKRD  equ 2
;
%include "version2.inc"		; include file "version2.inc"
...
%endif
    db '2.0'                ; Always define "2.0"
;    db '3.3'                ; Always define "3.3"
...
; NOT USE: Zerood out Extended BPB
;					db	0			; ONE zero for PhyDrv (Offset 0x24)
;;	PhyDrv	  		db	0x80		; MOVE to 1FDh
;					db	0			; ONE zero for CURHD (Offset 0x25)
;;	CURHD			db  0			; MOVE to 1FCh Current Head Number = Same field as ReservedNT 
;;   Zero out other fields
;					db    0			; one zero for Ext_Boot_Sig
;					dd    0			; four zeros for Boot_Serial
;					dd    0			; four zeros for Boot_Vol_Label
;					dd    0			; four zeros for Boot_Vol_Label
;					dw    0			; two zeros for Boot_Vol_Label
;					db    0			; one zero for Boot_Vol_Label
;;	Boot_Vol_Label	db    'NO NAME    ' ; AN000; 
;					dd    0			; four zeros for Boot_System_id
;					dd    0			; four zeros for Boot_System_id
...
CURHD	      	equ UDATA+17      	; Stores the current sector for disk reads.
CURSEC      	equ UDATA+22      	; Stores the current sector for disk reads.
;CURSEC      	equ UDATA+17      	; Stores the current sector for disk reads.
DIR$_L       	equ UDATA+26      	; Stores the low word of the root directory's starting sector LBA.
DIR$_H       	equ UDATA+28      	; Stores the high word of the root directory's starting sector LBA.
...
; Test jump Skip + db 'FAT' at 0x36
; Test jump Skip + 3 nop's
	jmp 		Skip
; Test nop's
;	nop
;	nop
...
; NEXT line not needed? Not there in MS-DOS 5 boot code! (DOS follows BIO without any bytes between!)
;	mov        	si, DOS				; ORG: 'MOV	SI,OFFSET DOS' [OFFSET is impicit in NASM?] - point to "ibmdos  com"
;
	mov        	cx, 0xB				; 0xb = 11
	repe 		cmpsb 				; Compare CX bytes from DS:SI with ES:DI
;===New Label needed for New Code regarding BIO LoadSize===========================================================
	je			DoSize				; Jump to get numbers of sectors to load
;ORG;	je         	DoLoad
; There has been some recoverable error. Display a message and wait for a keystroke.
; next line does not seem to be really needed ??
	jb         	CKErr				; Points to Non-System Message
CKErr:
...
;    CF = 1: Operation failed (for instance read/write error on disk). 
...
;===New Code to check Filesize of BIO and calculate actual number of sectors of BIO to load if below IBMLOADSIZE===
; Low bytes of Filesize of BIO at [bx + 0x1C], max loadsize = 58 sectors of 512 Bytes per Sector [0x7C0B] = 0x7400 bytes
DoSize:
	mov        	cx, IBMLOADSIZE 	; Set IBMLOADSIZE (max) first
	mov			ax, word [bx+0x1E]	; Copy High Word of Filesize of BIO to AX
	cmp 		ax, 0				; Compare High Word of Filesize with zero
	ja			DoLoad				; Jump if AX is above 0 bytes
	mov			ax, word [bx+0x1C]	; Copy Low Word of Filesize of BIO to AX
	cmp 		ax, 0x7400			; Compare Low word of Filesize with max available between BioOff up to ORIGIN -/- room for stack
	ja			DoLoad				; Jump if AX is above 58 * 512 bytes
; Next two lines for rounding up of sectors
	add			ax, [ByteSec]		; Set AX +BytSec
	dec			ax					; Set AX -1
	xor			dx, dx				; Zero DX =>  prepare for division by ByteSec
	div			word [ByteSec]		; Divide filesize by BytSec; quotient is in AX (implicit), DX =0 so no overflow possible
	mov			cx,	ax				; Set CX to real number of sectors used by BIO
;OKE;	mov			[0x7DFA], ax		; Debug New Code: result of number of calculated sectors to load if 58 or less, written to offset 1FAh
;===End of New Code regarding BIO LoadSize=========================================================================
DoLoad:
	mov 		bx, BioOff			; Set BX = Offset of IBMBIO (IBMLOAD) to be loaded.
;ORG;	mov        	cx, IBMLOADSIZE 	; # of sectors to read. Offset of ibmbio(IBMLOAD) to be loaded.
...
DISKOK:
; No difference: in loading from Harddrive with cleaning, written back by IO.SYS v 3.31?
;	xor			ax,	ax				; set AX=0
;	mov			word [cTotSec_L], ax		; Clean TotSec32 (assume not used!)
;	mov			word [cTotSec_H], ax		; Clean TotSec32 (assume not used!)
; Debug in Emulator: Next three lines equal values as with MSBOOT40.bin
...
; commenting out: 'mov ax, word [BIOS$_H]' makes no difference for HDD-boot, floppie still OKE. Emu: AX=0x0000 in MSBOOT40.bin
...
; Final jump to handle execution to IO.SYS: with MSBOOT40_2_331.bin and with HDD 'ljmp 0x70:0' in Emulator. IO.SYS is somewhere angry?
...
%endif 								;	ENDIF
;
	times 509 - ($ - $$) db 0   	; Pad remainder of boot sector with 0 ; From v027
;ORG;	times 508 - ($ - $$) db 0   	; Pad remainder of boot sector with 0 ; From v027
;
;ORG;	CURHD			db  0			; Current Head Number
