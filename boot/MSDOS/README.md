...
Features: auto-boot of MS-DOS v4-v7 IO.SYS; loads MS-DOS v7 IO.SYS if detected; but first four sectors of IO.SYS must be contiguous (order of System File root-directory entries must same as with MS-DOS v4-v6); address bp+0x1EE carries address of start of Message Table (MS-DOS v7 LDOS load protocol); writes after every sector read a dot on screen; further same as MSBOOT50  
