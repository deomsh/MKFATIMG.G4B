...
;
; MSBOOT5070.nasm, based on MSBOOT702.24.nasm = original MASM: MSBOOT.ASM converted to NASM with changes for MS-DOS v7 FAT12/16 load protocol.
; v0.13 (20251229), by deomsh, with help of Gemini 2.5 PRO, DeepSeek v3 and Copilot Smart (GPT-5).
; Boot Code should be compatible with MS-DOS 5.0-7.0 (MS-DOS 4.0 is special case).
; Always BIO must be the FIRST and DOS the SECOND Directory Entry in the Root Directory.
; Except for MS-DOS 4.0, BIO may start at a higher Cluster number than 2, first FOUR sectors must be contiguous.
;
...
;=============================================================================================
; Changes for NASM-compiler: MSBOOT40.nasm, by deomsh
; WORKING 2025 Nov, 05: second message had to be shortend (in boot40.cl1)
;
;=============================================================================================
; Changes for combined boot protocols MS-DOS v6/7 (FAT12/16): MSBOOT701.nasm, by deomsh
; WORKING 2025 Nov, 09: auto MS-DOS v6 or v7. Cost: many bytes of error-message => boot701.cl1
; IO.SYS and MSDOS.SYS must still be on disk according to MS-DOS v6 assumptions!
; 
; IBMLOADSIZE equ 4
;
; DISKOK:
...
ORIGIN 				equ 	0x7C00	; Origin of bootstrap LOADER.
BIOSEG 				equ		0x70 	; Destingation segment of BIOS.
BioOff 				equ		0x700 	; Offset of bios.
;
;UNUSED;BioOff2				equ		0x900 	; Offset of bios.
;
cbSec 				equ		512		; Bytes per Sector.
cbDirEnt 			equ		32 		; Bytes per directory entry.
DirOff	    		equ		0x500 	; The first [!root-] directory sector is loaded into 00500h
; NEW: for MS-DOS v7 load protocol
IBMLOADSIZE			equ		4		; needed for MS-DOS v7 - four sectors of IO.SYS loaded (contiguous!)
;ORG; IBMLOADSIZE			equ		3		; IBMLOADSIZE equ 3			;J.K. Size of IBMLOAD module in sectors
;
ROM_DISKRD			equ		2		; Set AH = 0x02 (BIOS disk read function), see: AC000:=2
;
;ORG;%include "version.inc"				; include file "version.inc"
;
...
;
BIOS$_L_OFFSET	equ START-$$+11	; Stores the low word of the data area's starting sector LBA.
BIOS$_H_OFFSET	equ START-$$+13	; Stores the high word of the data area's starting sector LBA.
CURTRK_OFFSET	equ START-$$+15	; Stores the current track/cylinder for disk reads.
CURSEC_OFFSET	equ START-$$+17	; Stores the current sector for disk reads.
DIR$_L_OFFSET	equ START-$$+18	; Stores the low word of the root directory's starting sector LBA.
DIR$_H_OFFSET	equ START-$$+20	; Stores the high word of the root directory's starting sector LBA.
CLUSNUM_OFFSET	equ START-$$+22	; Stores the low word of first Cluster Number of BIO.
;
SYSMSGOFFSET	equ SYSMSG-$$	; BIO v7: 0x1ee
;
...
;ORG;%if IBMCOPYRIGHT	          		; Check if ibmcopyright is defined
;ORG;    db 'IBM  '               		; If true, define "IBM  "
;ORG;%else
    db 'MSDOS'               		; If false, define "MSDOS"
;ORG;%endif
    db '5.0'                 		; Always define "5.0"
;
%define	ByteSec		bp+0xb
	dw	cbSec      	; Bytes per Sector ; ByteSec   DW	  cbSec 	; SIZE OF A PHYSICAL SECTOR
%define	SecClus		bp+0xd
	db	8			; DB	  8			; SECTORS PER ALLOCATION UNIT
%define	cSecRes		bp+0xe
	dw	1			; cSecRes   DW	  1			; NUMBER OF RESERVED SECTORS
%define	cFat		bp+0x10
	db	2			; cFat	  DB	  2			; NUMBER OF FATS
%define	DirNum		bp+0x11
	dw 	512			; DirNum	  DW	  512			; NUMBER OF DIREC ENTRIES
%define	cTotSec		bp+0x13
	dw	4*17*305-1	; cTotSec   DW	  4*17*305-1		; NUMBER OF SECTORS - NUMBER OF HIDDEN SECTORS
									;  (0 when 32 bit sector number)
%define	MEDIA		bp+0x15
	db 	0xF8		; MEDIA	  DB	  0F8H			; MEDIA BYTE
%define	cSecFat		bp+0x16
	dw	8			; cSecFat   DW	  8			; NUMBER OF FAT SECTORS
%define	SECLIM		bp+0x18
	dw	17			; SECLIM	  DW	  17			; SECTORS PER TRACK
%define	HDLIM		bp+0x1A
	dw	4			; HDLIM	  DW	  4			; NUMBER OF SURFACES !HEADS
%define	cSecHid_L	bp+0x1c
	dw	63			; cSecHid_L DW	  1 	; Ext_cSecHid label dword AN000; NUMBER OF HIDDEN SECTORS
%define	cSecHid_H	bp+0x1e
	dw	0			; cSecHid_H dw	  0			;AN000; high order word of Hiden Sectors
%define	cTotSec_L	bp+0x20
	dw	0			; cTotSec_L dw	  0	; Ext_cTotSec label dword AN000; 32 bit version of NUMBER OF SECTORS
%define	cTotSec_H	bp+0x22
	dw	0
	; AN000; (when 16 bit version is zero)
%define	PhyDrv		bp+0x24
	db	0x80		; PhyDrv	  db	 80h			;AN004;
%define	CURHD		bp+0x25
	db  0			; Current Head Number
;Unused;%define	Ext_Boot_Sig	bp+0x26
	db    41		; Ext_Boot_Sig	db    41		;AN000;
;Unused;%define	Boot_Serial	bp+0x27
	dd    0			; Boot_Serial	dd    0 		;AN000;
;Unused;%define	Boot_Vol_Label	bp+0x2b
	db    'NO NAME    ' ; AN000; 
;Unused;%define	Boot_System_id	bp+0x36
	db    'FAT12   '	; AN000; 
;
...
;ORG; Sec9        	equ UDATA+0       	; A temporary storage area for the disk parameter table.
;v12;BIOS$_L      	equ UDATA+11      	; Stores the low word of the data area's starting sector LBA.
;v12;BIOS$_H      	equ UDATA+13      	; Stores the high word of the data area's starting sector LBA.
;v12;CURTRK      	equ UDATA+15      	; Stores the current track/cylinder for disk reads.
;v12;CURSEC      	equ UDATA+17      	; Stores the current sector for disk reads.
;v12;DIR$_L       	equ UDATA+18      	; Stores the low word of the root directory's starting sector LBA.
;v12;DIR$_H       	equ UDATA+20      	; Stores the high word of the root directory's starting sector LBA.
;
;v12;CLUSNUM       	equ UDATA+22      	; Stores first Cluster Number of BIO as word.
;
START:
...
; 	ASSUME	SS:CODE					; NOT used in NASM
    ...
    mov                 bp, sp              ; 2 bytes instead of three later (needed by MS-DOS 7 Load Protocol)
...
; Test without relocating DPT
;org;	mov        	bx, DSKADR			; Set BX = 0x78
;org;	lds        	si, ss:[bx]			; Load DS:SI from SS:BX
;org;	push       	ds					; Push DS onto the stack
;org;	push       	si					; Push SI onto the stack
;org;	push       	ss					; Push SS onto the stack
;org;	push       	bx					; Push BX onto the stack
...
;org;	mov        	di, Sec9			; DeepSeek: DI now points to Sec9 (UDATA+0) as destination. offset in 'MOV	DI,offset Sec9' is implicit.
;org;	mov        	cx, 0xB				; DeepSeek: Sets CX to the number of bytes to copy (11)
;org;	cld								; DeepSeek: cld Ensures MOVSB processes data in the forward direction
...
;org;	rep movsb						; AN000; NASM assumes DS for the source
...
; Next two lines maybe still needed her
; Test to left next two lines out too
;org;	push       	es					; Push ES onto the stack
;org;	pop        	ds					; AN000; Pop SS into DS	
...
;org;	mov         byte [di - 2], 0xF	; Updates the segment part of the disk parameter table vector at SS:BX+2
;org;	mov    	    cx, word [SECLIM]	; AN004; Set CX = SECLIM (SECTORS PER TRACK). Loads the sectors per track value into CX
;org;	mov        	byte [di - 7], cl	; Update disk parameter table with SECLIM
; Place in new disk parameter table vector.
;org;	mov	        word [bx + 2], ax	; Updates the segment part of the disk parameter table vector at SS:BX+2
;org;	mov         word [bx], Sec9		; Updates the offset part of the disk parameter table vector at SS:BX.
...
Dir_Cont:
	...
	mov        	word [bp+DIR$_L_OFFSET], ax	; DX;AX = cFat*cSecFat + cSecRes + cSecHid. Save AX (lower word) in DIR$_L. 
	mov        	word [bp+DIR$_H_OFFSET], dx	; AN000; Save DX (higher word) in DIR$_H. 
	mov        	word [bp+BIOS$_L_OFFSET], ax	; DeepSeek: writes the (!lower) 16-bit value in AX to the memory location specified by BIOS$_L
; A 32-bit value is typically stored in a pair of registers (e.g., DX:AX in 16-bit mode, where DX holds the upper 16 bits and AX holds the lower 16 bits)
	mov        	word [bp+BIOS$_H_OFFSET], dx	; AN000; DeepSeek: Upper 16 bits stored in BIOS$_H
; Take into account size of directory (only know number of directory entries)
	...
	add        	word [bp+BIOS$_L_OFFSET], ax	; Start sector # of Data area. DeepSeek: Adds the value in AX to the 16-bit value stored at the memory location [BIOS$_L]
...
;	The result is stored in the memory location [BIOS$_H]
	adc        	word [bp+BIOS$_H_OFFSET], 0	; AN000; adds the Carry Flag (CF) (!from previous calculation) to [BIOS$_H].
...
; We load in the first directory sector and examine it to make sure the the BIOS and DOS are the first two directory entries. If they are not found, the user is prompted to insert a new disk. The directory sector is loaded into 00500h
	...
	mov        	dx, word [bp+DIR$_H_OFFSET]	; AN000;
	mov        	ax, word [bp+DIR$_L_OFFSET]	; logical sector of directory
...
;org;	pop 		si					; reset disk parameter table back to rom. Pop the saved SI value from the stack. SI points to the disk parameter table. 
;org;	pop 		ds					; Pop the saved DS value from the stack. DS is the segment for the disk parameter table. 
;org;	pop        	word [si]			; Pop the saved offset of the disk parameter table into [SI]. 
;org;	pop        	word [si + 2]		; Pop the saved segment of the disk parameter table into [SI + 2]. 
...
DoLoad:
;===New Code: Test First
	mov        	ax, word [bx + 0x1a]	; Read Cluster Number of BIO
	mov        	word [bp+CLUSNUM_OFFSET], ax	; Store Cluster Number of BIO in UDATA+22
; Two lines above are oke
	dec        	ax					; Decrease by one
	dec        	ax					; Decrease by one
	mov        	bl, byte [SecClus]	; Load numbers of SectorsPerCluster in BL
	xor        	bh, bh				; Zeroing BH
	mul        	bx					; Multiply BL with AX to get sector-offset in data-area
	add        	ax, word [bp+BIOS$_L_OFFSET]	; Add to AX low word of Start sector of Data area (BIOS$_L]
	adc        	dx, word [bp+BIOS$_H_OFFSET]	; Add to DX high word + carry of Start sector of Data area (BIOS$_H]
;===ORG
...
; Move and do not use push ax & pop ax
	mov        	bx, word [bp+BIOS$_L_OFFSET]	; Load lower 16 bits into BX. AN000; J.K.I1.Get bios sector in bx
	mov        	ax, word [bp+BIOS$_H_OFFSET]	; Overwrite AX with higher 16 bits (AX now contains only BIOS$_R). AN000; J.K.I1.
	push		ax					; Push ax to stack for now (ch and dl are in use!). Really needed because of changed compare later?
;===NEW: for choice of MS-DOS v6 & v7 load protocol [lines by Copilot & GLM 4.6]===
; Test earlier 
	mov			di, word [bp+CLUSNUM_OFFSET]	; set Cluster number to 2 - true if BIO starts on first cluster = Cluster 2.
	cmp			word [cs:0x0700], 0x5a4d	; word must be specified (= 'MZ')
	je			MSDOS7    			; If cmp was equal: continue to load IO.SYS from MS-DOS v7, otherwise do V6-jump
	jmp 		BIOSEG:0x0000 		; MS-DOS v6: Far jump to BIOS entry point (BIOSEG = 0x70). !BioOff is 0x700 - can not be used here ;0x0000000000000142:  EA 00 00 70 00    ljmp       0x70:0							; JMP	FAR PTR BIOS		;CRANK UP THE DOS
;
	jmp			BADJUMP
;
MSDOS7:                             ; Include lines for MS-DOS v7 load protocol
;
	cmp			word [cs:0x0900], 0x4a42	; word must be specified (= 'BJ')
	jne			CKErr
;
	mov         [ss:bp-4], bx		; ms-dos v7 load protocol expects first data sector at sp:bp-4 (low word in this case)
	mov         [ss:bp-2], ax		; same for higher 16 bits
BADJUMP:
        mov             ax, 0
        mov             ds, ax
        jmp             CKErr
...
DODIV:	; convert a logical sector into Track/sector/head.  AX has the logical sector number. J.K. DX;AX has the sector number. Because of not enough space, we are going to use Simple 32 bit division here. Carry set if DX;AX is too big to handle.
	...
	mov        	byte [bp+CURSEC_OFFSET], dl	; Save DL (sector number) in CURSEC. Cursec is 1-based. DeepSeek: Many BIOS routines and disk formats (e.g., floppy disks) use 1-based sector numbering (!instead starting with 0).
	...
	mov        	word [bp+CURTRK_OFFSET], ax	; Save AX (track number) in CURTRK. 
...
DOCALL:	; The DOCALL subroutine prepares and executes a BIOS disk read operation. It combines track, sector, head, and drive information for the BIOS call.  
	...
	mov        	dx, word [bp+CURTRK_OFFSET]	; Load DX with the current track number. 
...
; Common Use Cases: Setting Specific Bits; Merging Data; Fast Zero Check.
	or         	dh, byte [bp+CURSEC_OFFSET]	; Combine DH (track) with CURSEC (sector number). 
...
; END of new Code for printing Dot
	ret								; Return from subroutine. 
;
SYSMSG		db 0x0d, 0x0a, 0x0a, "Non-System disk or disk error"
			db 0x0d, 0x0a, "Replace and press any key when ready..."
NewLines    db 0x0d, 0x0a, 0x0a, 0
;
;ORG;%include "boot5070.cl1" 				; Data area for messages and filenames in file "boot5070.cl1", scond message only 2 bytes shortened
;%include "boot4070.cl1" 				; Data area for messages and filenames in file "boot4070.cl1", both messages shortened
; %include "boot70.cl1" 				; Data area for messages and filenames in file "boot70.cl1", second message shortened
; 
;%include "boot.cl1" 				; Data area for messages and filenames in file "boot.cl1", see AN003
									; Original:
;ORG;%if IBMCOPYRIGHT 					;   IF IBMCOPYRIGHT
;ORG;BIO db "IBMBIO  COM" 				; BIO	DB	"IBMBIO  COM"
;ORG;DOS db "IBMDOS  COM" 				; DOS	DB	"IBMDOS  COM"
;ORG;%else 								; 	ELSE
BIO db "IO      SYS" 				; BIO	DB	"IO      SYS"
DOS db "MSDOS   SYS" 				; DOS	DB	"MSDOS   SYS"
;ORG;%endif 								;   ENDIF
;
    times 494 - ($ - $$) db 0   	; Pad remainder of boot sector with 0 ; From v027
	dw	SYSMSGOFFSET				; BIO v7: 0x1EE
;	db	0
;
;    times 497 - ($ - $$) db 0   	; Pad remainder of boot sector with 0 ; From v027
;
WRITE:	; The code is a loop that prints characters from memory (DS:SI) using BIOS INT 0x10 teletype function. It stops when a null terminator (\0) is encountered. 
	...
	je         	ENDWRITE			; Jump to ENDWR if AL = 0 (end of string), ERROR MESSAGE UP, JUMP TO BASIC
	mov        	ah, 0xE				; Set AH = 0x0E (BIOS teletype function), WILL WRITE CHARACTER & ATTRIBUTE
;ORG;	mov        	bx, 7				; Set BX = 7 (attribute: white on black). Sets the video attribute byte for BIOS INT 0x10 teletype output.
...
ENDWRITE:
	ret
;
    times 508 - ($ - $$) db 0   	; Pad remainder of boot sector with 0 ; From v027
;ORG;    times 510 - ($ - $$) db 0   	; Pad remainder of boot sector with 0 ; From v027
;
    db 0
    db 0
