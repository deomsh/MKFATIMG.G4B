; Modified by deomsh in 2026
    ; Original code from MS-DOS 4.0 (c) Microsoft/IBM
    ; Licensed under the MIT License
;
; MSBOOT.nasm = original MASM: MSBOOT.ASM converted to NASM
; (20260102), by deomsh, with help of Gemini 2.5 PRO, DeepSeek v3 and Copilot (GPT-5)
...
;NEW;    mov 		ax, cs              ; Copy CS to AX (8C C8) instead of MASM:   ASSUME	SS:CODE
;NEW;    mov 		ss, ax              ; Set SS = CS (8E D0)
...
;NEW;	mov 		sp, ORIGIN 			; Sets the stack pointer (SP) to ORIGIN (i.e. 0x7C00). 
;NEW;	xor     	ax, ax				; Clear AX. Prepares AX for use in setting up segment registers. Clears all arithmetic flags (ZF=1, CF=0, etc.). Smaller and faster than mov ax, 0 (2 bytes vs 3 bytes).
;NEW;    mov 		ds, ax              ; Set DS = 0x0000
;NEW;    mov 		es, ax              ; Set ES = 0x0000
;LATER;    sti         		            ; Re-enable interrupts
; END of new code XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX;
; For now original code !!
	xor        	ax, ax
