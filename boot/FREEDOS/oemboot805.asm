...
;	Memory Map updated
; Used with MSCOMPAT or WINBOOT and MSCOMPAT, other OS's not tested
;
...
;	+--------+
;	| FAT16  |
;	| BUFFER | also used as FAT buffer !!! ONLY in case of FAT16 !!!
;	|--------| 2000:0000
;	|--------|
;	| CLUSTER|
;	|  LIST  |
...
;	|LOADED  | also used as FAT buffer !!! ONLY in case of FAT12 !!!
;	|--------| 0070:0000 (0:0700)
...
%define FATBUF          bp-0x7500       ; offset of temporary buffer for FAT12
%define FATSEG          0x2000          ; segment value of buffer for FAT16
...
;
                ...
                mov     al, [bsFATs]
;ORG:                cbw                        ; NOT needed, AH is already 0
...
%else
				mov     ax, FATSEG              ; Our new, safe segment
;v800;				mov     ax, 0x1000              ; Our new, safe segment
				mov     es, ax
...
fat_16:         mov     dx, FATSEG      ; Point DX to our new, safe segment [inserted by Gemini 2.5 Pro]
;v800;fat_16:         mov     dx, 0x1000      ; Point DX to our new, safe segment [inserted by Gemini 2.5 Pro]
...
;
                mov dl, [drive]         ; NEEDED: DOSv6 Load Protocol
;WRONG;                                       ; dl set to drive by readDisk
...
;
;                db      "E."                    ; Saves ONE byte (not in case of FAT12 only if cbw is skipped)
;ORG;
                db      "):."
;
%ifdef LOOPONERR
